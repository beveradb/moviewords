...
```
 ...
 │ download → curate → index → count → enrich/credits → derive  │
 └───────────────┬──────────────────────────────────────────────┘
...
| `json/movie/<id>.json` | stats + top/top_all/distinctive; entries `[word, value, zipf, classes]`; `quality: {tier: "low", flags}` on films left out of the aggregates | movie page (no WASM needed) |
...
| `json/movies-index.json` | slim all-movies list (search index); `q: "low"` marks films left out of the aggregates - client-side counts skip them (`inCorpus`) | client-side search |
| `movies_flagged.parquet`, `words_by_movie_flagged/data.parquet` | same schemas (+ `quality`, `quality_flags`) for films whose only subtitle is low quality - their film pages only, never an aggregate | film pages |
...
single-letter noise dropped except a/i; hyphenated words split; in OCR-damaged
files (3+ tokens like `l'm`, `i'ii`), I/l and ll/ii confusions repaired when the
fix is a far commoner word. **Only bags of
...
IMDb id; TMDB `original_language == en` (translated subtitles measure
translators, not screenwriters - both corpora are published, and the toggle
to switch between them ships in the app).

**Choosing a film's subtitle.** An OPUS folder holds one file per upload. The
index keeps files in a plausible words-per-minute band and samples up to 12;
the count stage fingerprints each and takes the most typical file of the
largest group of agreeing texts (`consensus.py`,
`docs/superpowers/plans/2026-09-24-content-consensus-selection.md`), skipping
commentary tracks, other languages, and files flagged as auto-captions,
machine-translated, naming none of the cast, or with anachronistic profanity
(`quality.py`, `docs/superpowers/plans/2026-09-25-subtitle-quality.md`). A film
whose every file is flagged keeps its page but is left out of every aggregate
(quality tier "low"); `scripts/audit_quality.py` runs the canaries.
...
| Layer | Tools |
|---|---|
| Pipeline | Python 3.12, uv, duckdb, pyarrow, requests, wordfreq, nltk (WordNet), pytest (230+ tests incl. fixture-corpus e2e) |
