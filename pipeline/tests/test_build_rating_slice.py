...
def test_build_rating_slice_filters_films_and_rederives_word_year(tmp_path):
    ...
    # tt6 (nc17, 1995) folds into the 'r' slice via SLICE_MEMBERS
    assert n == 4
    assert duckdb.sql(f"SELECT imdb_id FROM '{out_in}/movies.parquet' ORDER BY 1").fetchall() == \
        [("tt1",), ("tt2",), ("tt3",), ("tt6",)]
    assert duckdb.sql(f"SELECT count(*) FROM '{out_in}/words_by_movie.parquet'").fetchone()[0] == 5
    ...
    ).fetchall() == [("fuck", 1990, 25, 2), ("fuck", 1995, 50, 2)]
...
def test_build_rating_slice_rejects_nc17_as_its_own_slice(tmp_path):
    from build_rating_slice import build_rating_slice
    all_in = _fixture(tmp_path)
    with pytest.raises(ValueError):
        build_rating_slice(all_in, all_in / "rating" / "nc17", all_in / "ratings.parquet", "nc17")
